...
```{r prepBoxplot}
box_fig <- ggplot(cars, aes(x=origin, y=mpg)) + geom_boxplot() + theme_bw()
```
...
```{r prepRegPlot}
reg_fig <- ggplot(cars, aes(x=wt, y=mpg, colour=origin, text=rownames(cars))) + geom_point(size=2) +
